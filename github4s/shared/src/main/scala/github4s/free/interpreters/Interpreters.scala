...
class Interpreters[M[_], C](implicit A: ApplicativeError[M, Throwable],
  ...
  implicit def interpreters(
    ...
    val c01interpreter
      : COGH01 ~> K = repositoryOpsInterpreter or userOpsInterpreter
    ...
    val c06interpreter
      : COGH06 ~> K = pullRequestOpsInterpreter or c05interpreter
  ...
  def repositoryOpsInterpreter: RepositoryOp ~> K = new (RepositoryOp ~> K) {
    ...
    def apply[A](fa: RepositoryOp[A]): K[A] =
      Kleisli[M, Map[String, String], A] { headers =>
        fa match {
          ...
          case ListBranches(owner, repo, branchProtected, accessToken) ⇒
            repos
              .listBranches(accessToken, headers, owner, repo, branchProtected)
          case ListContributors(owner, repo, anon, accessToken, pagination) ⇒
          ...
          case ListCollaborators(owner, repo, affiliation, accessToken) =>
            repos
              .listCollaborators(accessToken, headers, owner, repo, affiliation)
          case CreateRelease(
              owner,
              repo,
              tagName,
  ...
  def userOpsInterpreter: UserOp ~> K =
    new (UserOp ~> K) {
      ...
      def apply[A](fa: UserOp[A]): K[A] = Kleisli[M, Map[String, String], A] {
        headers =>
          fa match {
            case GetUser(username, accessToken) ⇒
              users.get(accessToken, headers, username)
            ...
            case GetFollowing(username, accessToken) =>
              users.getFollowing(accessToken, headers, username)
          }
      }
    }
  ...
  def authOpsInterpreter: AuthOp ~> K =
    new (AuthOp ~> K) {
      ...
      def apply[A](fa: AuthOp[A]): K[A] = Kleisli[M, Map[String, String], A] {
        headers =>
          fa match {
            case NewAuth(
                username,
  ...
  def gistOpsInterpreter: GistOp ~> K =
    new (GistOp ~> K) {
      ...
      def apply[A](fa: GistOp[A]): K[A] = Kleisli[M, Map[String, String], A] {
        headers =>
          fa match {
  ...
  def issueOpsInterpreter: IssueOp ~> K =
    new (IssueOp ~> K) {
      ...
      def apply[A](fa: IssueOp[A]): K[A] = Kleisli[M, Map[String, String], A] {
        headers =>
          fa match {
            ...
            case CreateComment(owner, repo, number, body, accessToken) ⇒
              issues
                .createComment(accessToken, headers, owner, repo, number, body)
            ...
            case AddLabels(owner, repo, number, labels, accessToken) ⇒
              issues
                .addLabels(accessToken, headers, owner, repo, number, labels)
            case RemoveLabel(owner, repo, number, label, accessToken) ⇒
              issues
                .removeLabel(accessToken, headers, owner, repo, number, label)
  ...
  def activityOpsInterpreter: ActivityOp ~> K =
    new (ActivityOp ~> K) {
      ...
      def apply[A](fa: ActivityOp[A]): K[A] =
        Kleisli[M, Map[String, String], A] { headers =>
          fa match {
  ...
  def gitDataOpsInterpreter: GitDataOp ~> K =
    new (GitDataOp ~> K) {
      ...
      def apply[A](fa: GitDataOp[A]): K[A] =
        Kleisli[M, Map[String, String], A] { headers =>
          fa match {
  ...
  def pullRequestOpsInterpreter: PullRequestOp ~> K =
    new (PullRequestOp ~> K) {
      ...
      def apply[A](fa: PullRequestOp[A]): K[A] =
        Kleisli[M, Map[String, String], A] { headers =>
          fa match {
  ...
  def organizationOpsInterpreter: OrganizationOp ~> K =
    new (OrganizationOp ~> K) {
      ...
      def apply[A](fa: OrganizationOp[A]): K[A] =
        Kleisli[M, Map[String, String], A] { headers =>
          fa match {
            ...
            case ListOutsideCollaborators(
              ...
              organizations.listOutsideCollaborators(
                accessToken,
                headers,
                org,
                filter,
