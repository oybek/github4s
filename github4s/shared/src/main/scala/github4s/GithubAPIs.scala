...
class GHUsers(accessToken: Option[String] = None)(
  implicit O: UserOps[GitHub4s]
) {

  def get(username: String): GHIO[GHResponse[User]] =
    O.getUser(username, accessToken)
...
class GHRepos(accessToken: Option[String] = None)(
  implicit O: RepositoryOps[GitHub4s]
) {
  ...
  ): GHIO[GHResponse[NonEmptyList[Content]]] =
  ...
  def listContributors(
    ...
    O.listContributors(owner, repo, anon, accessToken, pagination)
  ...
  def getCombinedStatus(owner: String,
                        repo: String,
                        ref: String): GHIO[GHResponse[CombinedStatus]] =
    O.getCombinedStatus(owner, repo, ref, accessToken)

  def listStatuses(owner: String,
                   repo: String,
                   ref: String): GHIO[GHResponse[List[Status]]] =
    O.listStatuses(owner, repo, ref, accessToken)

  def createStatus(owner: String,
                   repo: String,
                   sha: String,
                   state: String,
                   target_url: Option[String] = None,
                   description: Option[String] = None,
                   context: Option[String] = None): GHIO[GHResponse[Status]] =
    O.createStatus(
      owner,
      repo,
      sha,
...
class GHAuth(accessToken: Option[String] = None)(
  implicit O: AuthOps[GitHub4s]
) {

  def newAuth(username: String,
              password: String,
              scopes: List[String],
              note: String,
              client_id: String,
...
class GHGists(accessToken: Option[String] = None)(
  implicit O: GistOps[GitHub4s]
) {
  def newGist(description: String,
              public: Boolean,
              files: Map[String, GistFile]): GHIO[GHResponse[Gist]] =
    O.newGist(description, public, files, accessToken)

  def getGist(gistId: String,
              sha: Option[String] = None): GHIO[GHResponse[Gist]] =
...
class GHIssues(accessToken: Option[String] = None)(
  implicit O: IssueOps[GitHub4s]
) {

  def listIssues(owner: String, repo: String): GHIO[GHResponse[List[Issue]]] =
    O.listIssues(owner, repo, accessToken)

  def getIssue(owner: String,
               repo: String,
               number: Int): GHIO[GHResponse[Issue]] =
  ...
  def editIssue(owner: String,
                ...
                milestone: Option[Int] = None,
                labels: List[String] = List.empty,
                assignees: List[String] = List.empty): GHIO[GHResponse[Issue]] =
    O.editIssue(
      owner,
      repo,
      issue,
  ...
  def listComments(owner: String,
                   repo: String,
                   number: Int): GHIO[GHResponse[List[Comment]]] =
    O.listComments(owner, repo, number, accessToken)

  def createComment(owner: String,
                    repo: String,
                    number: Int,
                    body: String): GHIO[GHResponse[Comment]] =
    O.createComment(owner, repo, number, body, accessToken)

  def editComment(owner: String,
                  repo: String,
                  id: Int,
                  body: String): GHIO[GHResponse[Comment]] =
    O.editComment(owner, repo, id, body, accessToken)

  def deleteComment(owner: String,
                    repo: String,
                    id: Int): GHIO[GHResponse[Unit]] =
    O.deleteComment(owner, repo, id, accessToken)

  def listLabels(owner: String,
                 repo: String,
                 number: Int): GHIO[GHResponse[List[Label]]] =
    O.listLabels(owner, repo, number, accessToken)

  def addLabels(owner: String,
                repo: String,
                number: Int,
                labels: List[String]): GHIO[GHResponse[List[Label]]] =
    O.addLabels(owner, repo, number, labels, accessToken)

  def removeLabel(owner: String,
                  repo: String,
                  number: Int,
                  label: String): GHIO[GHResponse[List[Label]]] =
...
class GHActivities(accessToken: Option[String] = None)(
  implicit O: ActivityOps[GitHub4s]
) {

  def setThreadSub(id: Int,
                   subscribed: Boolean,
...
class GHGitData(accessToken: Option[String] = None)(
  implicit O: GitDataOps[GitHub4s]
) {

  def getReference(owner: String,
                   repo: String,
                   ref: String): GHIO[GHResponse[NonEmptyList[Ref]]] =
    O.getReference(owner, repo, ref, accessToken)

  def createReference(owner: String,
                      repo: String,
                      ref: String,
                      sha: String): GHIO[GHResponse[Ref]] =
  ...
  def createTag(owner: String,
                ...
                objectSha: String,
                objectType: String,
                author: Option[RefAuthor] = None): GHIO[GHResponse[Tag]] =
    O.createTag(
      owner,
      repo,
      tag,
...
class GHPullRequests(accessToken: Option[String] = None)(
  implicit O: PullRequestOps[GitHub4s]
) {
  def get(owner: String,
          repo: String,
          number: Int): GHIO[GHResponse[PullRequest]] =
  ...
  def getReview(owner: String,
                repo: String,
                pullRequest: Int,
                review: Int): GHIO[GHResponse[PullRequestReview]] =
    O.getPullRequestReview(owner, repo, pullRequest, review, accessToken)
}

class GHOrganizations(accessToken: Option[String] = None)(
  implicit O: OrganizationOps[GitHub4s]
) {
